            ...
            <table class="text-sm text-left text-white w-full table-auto z-30">
                ...
                <tbody id="tableBody">
                    ...
                    <tr class="hover:bg-gray-800 bg-transparent">
                        ...
                        <!-- Matrícula -->
                        <td class="px-4 py-2 border-t border-blue-500 animate-border">{{ $AlumnoDelGrupo->Matricula }}</td>
                        
                        <!-- Nombre -->
                        <td class="px-4 py-2 border-t border-blue-500 animate-border text-center">
                            {{ $AlumnoDelGrupo->Nombre }} {{ $AlumnoDelGrupo->ApellidoPaterno }} {{ $AlumnoDelGrupo->ApellidoMaterno }}
                        </td>

                        <!-- CURP -->
                        <td class="px-4 py-2 border-t border-blue-500 animate-border">{{ $AlumnoDelGrupo->CURP }}</td>

                        <!-- Botones de Acción -->
                        <td class="px-4 py-2 border-t border-blue-500 animate-border text-center">
                            <form action="/CalificarAlumno/{{ $AlumnoDelGrupo->idAlumno }}" method="GET">
                                <button class="bg-red-600 text-white px-4 py-2 rounded-md hover:bg-blue-600">Calificar</button>
                            </form>
                        </td>
                        <td class="px-4 py-2 border-t border-blue-500 animate-border text-center">
                            <form action="/EnviarComunicado/{{ $AlumnoDelGrupo->idAlumno }}" method="GET">
                                <button class="bg-green-600 text-white px-4 py-2 rounded-md hover:bg-blue-600">Comunicado</button>
                            </form>
                        </td>
