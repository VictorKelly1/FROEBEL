<x-director.layout>
    ...
    <form class="posiciontablas bg-gray-900 p-4 rounded-md border border-blue-500 shadow-md w-full max-w-3xl mx-auto"
        ...
        <input type="text" name="Tipo" id="Tipo" class="form-control w-full p-3 mb-3 rounded-md text-lg"
            required>

        <!-- Botón de Envío -->
        <button type="submit"
            class="btn btn-primary bg-blue-600 text-white px-6 py-3 rounded-md hover:bg-blue-700 w-full mt-4">
            Asignar
        </button>
    </form>
    ...
    <div
        class="posiciontablasbaja flex flex-col items-center justify-center bg-gray-900 p-4 mt-4 rounded-md border border-blue-500 shadow-md md:w-3/4 lg:w-2/3 overflow-x-auto">
            ...
            <tbody id="tableBody">
                @foreach ($AlumTutor as $AlumnoTutor)
                    <tr class="hover:bg-gray-800 text-center">
                        ...
                        <td class="px-3 py-2 border-t border-blue-500">{{ $AlumnoTutor->NombreAlum }}
                            {{ $AlumnoTutor->ApellidoPatA }} {{ $AlumnoTutor->ApellidoMatA }}</td>
                        <td class="px-3 py-2 border-t border-blue-500">{{ $AlumnoTutor->NombreTutor }}
                            {{ $AlumnoTutor->ApellidoPaternoT }} {{ $AlumnoTutor->ApellidoMatT }}</td>
                        ...
                        <td class="px-3 py-2 border-t border-blue-500">
                            <form action="/AlumTutor/{{ $AlumnoTutor->idAlumnoTutor }}" method="GET">
                                <button
                                    class="bg-red-600 text-white px-4 py-2 rounded hover:bg-red-700">Desasignar</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
