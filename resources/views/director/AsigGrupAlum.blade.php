<x-director.layout>
    ...
    <div class="posiciontablasmedia flex items-center justify-center bg-gray-900 p-6 mt-6 rounded-md border border-blue-500 shadow-md w-3/4 sm:w-2/3 lg:w-1/2">
        <form class="w-full text-white text-base" action="{{ route('AsignarGrupAlum') }}" method="POST" enctype="multipart/form-data">
            @csrf

            <!-- 🧑 Alumno -->
            <div class="mb-4">
                <label for="Alumno" class="block mb-2 text-lg">Alumno:</label>
                <select name="idAlumno" id="Alumno" class="w-full px-4 py-2 rounded-md bg-gray-800 border border-blue-500 text-white text-lg" required>
